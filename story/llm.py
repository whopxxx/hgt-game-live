...
#:
#: ---- 2026-09-25: check-v11 -> check-v12(安全边界收窄: 五条 -> 两条) ----
#:
#: 运营决定: 边界只保留两条 —— ①以**性暴力**为核心情节; ②写**血腥、
#: 具体伤害的感官细节**(虐杀 / 肢解 / 具体伤口的描写)。原五条中的
#: **自伤 / 自杀**主题、**极端伤害当猎奇卖点**、**儿童 / 家庭严重暴力**
#: 三条, 与 SAFETY 复核(v2->v3)、QUALITY_POLICY(v12->v13)四处**同批移除**
#: —— 只改主审不改复核会让双门 AND 继续按旧口径卡题。
CHECK_PROMPT_VERSION = "check-v12"
...
class PuzzleWriter:
    ...
    **不再需要宽容解析**(只在工具调用不可用时才回退到文本解析)。
    ...
    def verify_safety(self, puzzle: str = "", answer: str = "",
        ...
        system 里**逐字复用** `_TOOL_CHECK` 的两条边界(性暴力 /
