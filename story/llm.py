...
#: **Story 阶段的方向提示** —— 极短, 两类各两句话。
#:
#: ⚠️ **不要在这里堆规则。** R2 已经证明"一行 `类型：黑汤。`"不足以定义
#: 风格(普通事故 / 亲情告别 / "有人死了"仍然大量混进黑汤), 但解法**不是**
#: 补十几条禁用模板 —— 那会重演 v1 把输出推成规范手册的老路。这里给的
#: 是**判据**(什么算黑暗/诡异), 不是**清单**(不许写哪些套路)。
STORY_LANE_DIRECTION = {
    "red": ("偏红汤：核心真相要有黑暗或危险的后果与认知反转。"
            "不能只靠\"有人死了\"充当揭晓。"),
    "black": ("偏黑汤：揭晓后要让原本的场景变得明显诡异、不安或错位。"
              "不能只是普通悲情、怀念或纪念。"),
}
...
只写事情真正发生了什么 —— 完整、具体、唯一的一个解释。
不要写谜面（汤面之后会由一步单独的截取产生）。
不要解释背景，不要写成摘要。
