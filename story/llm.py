...
5. 写 `core_answer`(一句话, ≤60 字, 不换行): 普通观众一听就懂的核心答案,
   必须**直接解释谜面的主要异常 / 核心悬念** —— 若谜面有显式问题就直接
   回答它; 没有显式问题时, 直接解释主要异常即可(谜面**不必**以问句结尾)。
...
答案就是 completion。判据是**删除测试**:
    ...
    观众**仍然已经能完整解释谜面的主要异常 / 核心悬念**, 那个细节就
    **不属于 completion**, 应当放进 support。
...
#:     core_answer_direct         core_answer 有没有正面解释谜面的主要异常
#:                                (有显式问题就回答它; **不要求谜面有问句**)
        ...
        "properties": {
            ...
            "core_answer": {
                ...
                "description": (
                    ...
                    "若谜面有显式问题, 就直接回答那个问题; 没有显式问题时, "
                    "只要直接解释了主要异常即可(谜面**不必**以问句结尾), "
                    "不能依赖额外脑补。推荐 <=60 汉字, 硬上限 80。"
                    "**不换行**。\n"
...
1. **谜面与谜底已经定了, 你改不了也不该改。** schema 里根本没有这两个
   字段 —— 不要试图"顺手润色一下"。
2. `core_answer` 必须**直接解释谜面的主要异常 / 核心悬念**。一句话,
   <=60 字, 不换行。若谜面有显式问题就直接回答它; 没有显式问题时,
   直接解释主要异常即可 —— 谜面**不必**以问句结尾。
        ...
        "properties": {
            "decision": {
                ...
                "description": (
                    ...
                    "fix     = 只做**局部修复**(某句泄底/提示剧透/措辞不清/"
                    "小范围 fact-atom 不一致/谜面混进【谜底】之类的元文本)。\n"
                    "⚠ **第一人称**与**结尾没有问句**都**不是**毛病, "
                    "不要因为它们选 fix。\n"
            ...
            "core_answer": {
                ...
                "description": (
                    ...
                    "解释谜面的主要异常 / 核心悬念 —— 有显式问题就回答它, "
                    "没有显式问题时直接解释主要异常即可)。pass 时原样回传。\n"
            ...
            "quality_checks": {
                ...
                "properties": {
                    ...
                    "core_answer_direct": {
                        "type": "boolean",
                        "description": (
                            "core_answer 普通人一句能懂, 且**直接解释谜面的"
                            "主要异常 / 核心悬念**(不绕、不依赖额外脑补)。\n"
                            "⚠ 谜面**不一定**有显式问句(例: \"每天晚上十二点, "
                            "她都会从门外听见自己敲门。\")。有显式问题时就"
                            "直接回答它; **没有显式问题时, 只要直接解释了"
                            "主要异常就算 true** —— 不要因为没有问句判 false。"),
                    },
                    "completion_contract_minimal": {
                        ...
                        "description": (
                            ...
                            "背景历史 -> false。\n"
                            "删除测试: 删掉该细节后观众仍能完整解释谜面的"
                            "主要异常 / 核心悬念, 就必须删除或降为 support。"),
                    },
                    "concrete_anomaly": {
                        "type": "boolean",
                        "description": (
...
core_answer 普通人**一句能懂**, 并且**直接解释谜面的主要异常 /
核心悬念**。绕圈子、"其实就是说……"才能明白的, 是 false。
⚠ 谜面**不一定**以问句结尾 —— 例: "每天晚上十二点, 她都会从门外
听见自己敲门。" 这种自明其问的汤面是**合法**的。有显式问题时, 就
直接回答它; **没有显式问题时, 只要直接解释了主要异常就算 true**。
不要因为"末尾没有问号"判这一项 false。
    ...
    **不再需要宽容解析**(只在工具调用不可用时才回退到文本解析)。
    ...
    def gen_spec(self, avoid: Optional[list] = None,
        ...
        while attempts < (max_attempts if check else 1) and guard < 8:
            ...
            # 审稿人没改掉, 不能再放行(否则那个真实缺陷会一路溜到直播上)。
            #
            # ⚠️ G4-RB: fixable 现在**只剩 meta 文本**(谜面混进【谜底】
            # 之类) —— 人称与"没有结尾问句"早就不再是毛病, 不会再走到
            # 这里。忘掉这条的话, 很容易把"合法但不带问号的汤面"
            # 错当成"没改掉的问题"。
