...
#: 送人工看 —— 宁可多送几条, 也不能让一条血腥细节直接上屏。
#: 真正细腻的判断留给 H2-B 的第 9 条("内容是否适合公开直播展示")
#: 与 LLM 双门(story/llm.py 的主审 + 复核)。
#:
#: 2026-09-25 收窄(与 story/llm.py 四处同批): 粗筛口径与 LLM 审查
#: 对齐, 只拦**血腥具体伤害**与**性暴力**两类。原 `self_harm`
#: (自杀 / 割腕 / 跳楼等)与原 `minor_harm` 里的**虐童**不再拦 ——
#: 自伤主题、儿童 / 家庭严重暴力已从 LLM 边界移除, 粗筛若继续拦,
#: 同一批题会"导不进来但审得过"。
_SAFETY_PATTERNS = (
    ("gore", ("碎尸", "肢解", "开膛", "挖出内脏", "脑浆", "血肉模糊", "剥皮")),
    ("sexual_violence", ("猥亵", "性侵", "强奸")),
)
