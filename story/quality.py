...
#:
#: ----------------------------------------------------------------------
#: R8-A: v11 -> v12(接受条件**再次**真的变了, 两个方向同时动)
#:
#: ## (a) 自由生成链: 九项门 -> 六项门
#:
#: `clue_recontextualized` / `dramatic_payoff` /
#: `reasoning_beats_nonredundant` 从**硬门**降成 **signal only**。
#: 它们仍由 Reviewer 回答、仍写进 `quality_checks`、仍落盘, 只是
#: **不再拒稿**。见 `_FREE_GEN_HARD_CHECK_FIELDS`。
#:
#: 动机是实播症状: 大量题被拒, 而留下来的题并没有更好。这三项问的是
#: "值不值得玩", 却当着"能不能过"的资格考试 —— 一道真实性/结构/安全
#: 都合格的题只因为"不够戏剧化"被整稿丢掉, 重出的下一稿只在正确性上
#: 掷骰子, 补池吞吐因此被压死。
#:
#:     v11: 九项全过                 -> 收
#:     v12: 六项全过(signal 不参与)  -> 收
#:
#: (b) safety 口径校准: 从"泛化不适感"收回到**明确五条边界**
#:
#: R7 的 safety 措辞里有"会不会变成拿惨案当乐子" / "宁可严一点 /
#: 拿不准时判 false" —— 这类泛化措辞会让模型自行扩张成
#: "死亡/亡魂/犯罪是核心 = 不安全", 把大量**本来完全可播**的凶案与
#: 灵异题材判死(海龟汤的类型特征)。
#:
#: 现在两处(主审 `livestream_safe` 描述 + `SAFETY_SYSTEM`)统一成:
#: 普通非血腥的死亡/犯罪/谋杀/亡魂/鬼怪/恐怖氛围 —— **即使是谜底
#: 核心** —— 不因此判 false; **只有**明确命中五条边界才 false;
#: **未明确命中 -> true**。同时删掉上述诱导泛化的措辞。
#:
#: ⚠️ 这不是"放宽成什么都能播": 五条边界一条没少, 双门 AND 也没删。
#: 收的是**泛化**那一半, 不是边界那一半。
#:
#: (a) 与 (b) 都会让**同一份 spec 的收/拒结果**与 v11 不同, 所以
#: 必须 bump。严格相等门会把 v11 库存隔离(行仍在盘上), 上线前需要
#: 一次 prewarm。
#:
#: ----------------------------------------------------------------------
#: 2026-09-25: v12 -> v13(安全边界收窄: 五条 -> 两条)
#:
#: 运营决定: 直播安全边界只保留两条 —— ①以**性暴力**为核心情节;
#: ②写**血腥、具体伤害的感官细节**(虐杀 / 肢解 / 具体伤口的描写)。
#: 原 v12 五条中的另外三条(**自伤 / 自杀**主题、**极端伤害当猎奇卖点**、
#: **儿童 / 家庭严重暴力**核心冲击点)从四处**同批移除**: 主审
#: `_TOOL_CHECK` 的 `livestream_safe` 描述、`CHECK_SYSTEM` 硬门清单、
#: `_review_spec` curated 分支、复核 `SAFETY_SYSTEM`。四处措辞同改,
#: 避免双门 AND 一边放宽一边仍按旧口径拒。
#:
#: 同一份 spec 在 v13 下可能 livestream_safe=true 而 v12 下 false
#: (例: 自伤主题的题), 收/拒结果变了, 所以 bump。严格相等门会把 v12
#: 库存隔离(行仍在盘上), 上线前需要一次 prewarm。
QUALITY_POLICY_VERSION = "quality-v13"
