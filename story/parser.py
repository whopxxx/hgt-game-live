...
# Issue #65: 正常业务裁决冻结为经典三态 是/不是/不重要;
# 「无关」从正常 enum 删除(不引入「不确定」)。
VERDICTS = ("是", "不是", "不重要", "揭晓")
# LLM 没能给出裁决时的**中性**结果 —— 绝不能说成业务裁决。
# "未判定"只是"系统这次没答上", 诚实且不误导观众(实测踩过:
# 模型超时被回成错误裁决, 把观众的思路带偏)。
# ⚠️ Issue #65: 「未判定」**只**来自技术失败(timeout / transport /
# 空 tool_input / schema invalid / 解析失败), 不是第四种业务答案。
UNAVAILABLE = "未判定"
SOLVE = "揭晓"
...
YES = "是"
NO = "不是"
#: "这个细节对谜题核心机制与通关没有实质影响"(Issue #65)。
#: 用于: 命题可理解、事实表未定义、且不影响解题 —— **不要**因为
#: "事实表没写"就判"不是"。历史名 `IRRELEVANT`(「无关」)已随
#: 三态收敛删除; A2 时代"候选却判无关"的自相矛盾重判同理退役。
UNIMPORTANT = "不重要"

# 按**优先级**排列的裁决关键词 —— 顺序敏感!
# '揭晓' > '不重要' > '不是' > '是'
# 若把 '是' 放前面, "不是" 会先命中 '是' 而误判。
# (legacy 关键词映射: 这张表只服务 legacy 自由文本回退链, 正常链
# 走结构化 tool。旧「无关」保留映射到「不重要」 —— 它是最接近的
# 三态语义, legacy archive 复盘不至于失真; 不代表「无关」复活。)
_VERDICT_PATTERNS: list[tuple[str, tuple[str, ...]]] = [
    (SOLVE, ("揭晓", "真相是", "答案是", "正确答案", "谜底是", "完全正确", "答对了")),
    (UNIMPORTANT, ("不重要", "无关", "没有关系", "没关系", "不相关",
                   "不相干", "无关紧要")),
